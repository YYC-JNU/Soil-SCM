...
> **创建日期**：2026-08-11
> **更新日期**：2026-08-21
> **当前版本**：v0.7.0（v0.7.x 工单 77 核心完成）；规划中 **v0.7.x 工单 78~81（地球化学深化：KNOBS 高离子强度收敛调优 / weathering 情景区分 / lime 盐基淋失强化 / HX log_k 微调重锚定）**
...
#### v0.5.3 — VGM + Feddes ET + 级联重构（状态迁移）✅ 已完成 (2026-08-19)
...
- [x] **van Genuchten-Mualem**：`LayerCascade` 状态从 `stored_water`（L/ha）迁移至
...
- [x] **VGM 参数三级优先级**：①`layer_overrides` 显式配置（`vgm_theta_r`/`vgm_alpha`/`vgm_n`）
...
- [x] **初始 θ 田间持水量校准**：废弃"50% 饱和"硬编码（物理审查：50% 饱和度反解 ψ≈-280cm 近
...
- [x] **Feddes ET**：Oudin (2005) 公式为主 + 固定气候态兜底（`latitude`/`pet_method`/
...
- [x] **LayerCascade 重构（下游接收能力）**：可排水量 = max(0, θ−θ_FC)×depth（θ_FC=θ(ψ=−100)
      与初始 θ 同源）；界面最大导水通量 = min(上下层 `ksat_drainage`)（木桶短板原理）；实际排水
      = min(可排水量, min(K_r(θ)·ksat_i, ksat_{i+1})×Δt×面积)；底部 L4 = 深层排水
- [x] **OM 矿化产 CO₂ 模块**：**加性**调制每层 GAS_PHASE pCO₂（`pCO₂_eff = base(T) + k_om×OM_i`，
      钳制 `≤pCO₂_max`，k_om=0.0005/pCO₂_max=0.05 → constants.py）；温度独立性（T 响应仅归 base 项）；
      4 层默认 OM 剖面 [30,15,8,5] 强化**表层**酸性；输出 `pCO2_eff` 诊断列
- [x] **输出扩展**：新增 `AET_mm`、`et_deficit_mm`、`soil_moisture_L1~L4`、`pCO2_eff` 列
      （验证水分平衡闭合）；`stored_water_Li` 列 L/ha 语义向后兼容
- [x] **验收实验集（E1~E3）（说明: E1 预平衡收敛 4.92 + 水分闭合 AET 935mm; E2/E3 pH 无方向响应, 省略, 详见 OPTIMIZATION_PLAN §7.6）**：E1 基线（4 层 15 年 natural）+ 预平衡收敛复验；E2 PET 敏感性扫描
...
| **v0.7.0** | 2026-08-21 | **地球化学重构**（NO₃⁻ 示踪池+水库串联淋失 D3 + 惰性阴离子 An⁻ 伴随淋失分级注入 + NH₄⁺ 等效置换 + D2 矿物风化集总注入（Arrhenius+降级）+ k_om E3 标定 + E2 PET 判别）；工单 70~75 ✅ + 调优 A+B+D（NH₄⁺量级 343 / HX log_k 2.8 / weathering 500+降级）；**324 测试全绿**；spec 69 + 工单 70~76（/grilling Q11~Q22 定案）；方向带验收进行中（natural 5.34 持平、fertilizer 8.1 从 11.4 改善未达 <4.0、lime 未回落）——30 年 8 情景全量 + `verify_v0_7_0_acceptance.py` |
| **v0.7.x** | 规划 | **地球化学深化**（工单 77~81）：工单 77 **REACTION 电荷平衡修复**（2026-08-21：实测证伪 GAS_PHASE 是 fertilizer 碱化主因，真因=裸注入电荷不平衡伪碱化；配对抗酸/盐基后单层施肥不碱化、sensitivity 3y 11.2→10.9 改善，333 测试）✅核心完成 + KNOBS 高离子强度收敛调优（30 年全量解锁）+ weathering 情景区分（natural off / fertilizer on）+ lime 盐基淋失强化 + HX log_k 微调重锚定；详见 HANDOFF §5.1 + `docs/analysis/V0_7_x_CHARGE_PAIRING.md` |
