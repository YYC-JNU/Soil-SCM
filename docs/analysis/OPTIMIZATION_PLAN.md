...
> **v0.6.0 目标（2026-08-18 更新，v0.6.0 已于 2026-08-19 完成）**：修复 v0.5.1 水文盒子的 4 个物理缺陷（surface_coeff 非物理补丁 / 水桶模型 / 缺 ET / 月度平滑抹杀），按 **v0.5.2 → v0.5.3 → v0.6.0** 三阶段推进水文物理化重构——Green-Ampt / VGM / Feddes ET / 化学子步长（已定案决策见 §7）；当前阶段 v0.7.x（工单 77 电荷平衡修复已核心完成，见 §九）
...
### 7.6 v0.5.3 落地执行日志（2026-08-19，工单 50~54）

**实施**：`/grilling`（Q1~Q13，3 轮 + 专家接缝审查 7 处补漏）→ `/to-spec`（spec 49）→ `/to-tickets`（50~54）→ `/implement`（TDD 红绿循环，逐工单提交）。

| 工单 | 内容 | 落地 |
|------|------|------|
| 50 | VGM 模块 + θ 状态迁移 | `src/vgm.py`（VGM 正反算/Mualem/三级参数/换算）+ `SoilState.theta` 迁移 + 初始 θ=vgm(−100) + 化学体积联动 + config 新字段（202 测试） |
| 51 | Feddes ET / Oudin PET | `calc_pet_oudin` 逐月数组 + `feddes_alpha(ψ)` 四阈值 + `apply_feddes_et` + config（223 测试） |
| 52 | LayerCascade 重构 + ET 集成 | θ_FC 可排水量 + K(θ) 界面 min + `calc_interface_flux` 纯向下 + ET 前置编排 + 删 50% 魔法数（226 测试） |
| 53 | OM 矿化产 CO₂ | `apply_om_pco2` 加性+钳制 + OM 剖面 [30,15,8,5] + 逐层 pCO₂ 调制（232 测试） |
| 54 | 集成 + 验收 + 发布 | 输出列（AET_mm/et_deficit_mm/soil_moisture/pCO2_eff）+ f0/fc 移除（breaking）+ 验收 E1~E3 + 发布（**234 测试**） |

**运行验证（E1：4 层 15 年 natural, seed=42）**：

| 指标 | v0.5.2 | v0.5.3 |
|------|--------|--------|
| 预平衡收敛 pH | — | **4.92（4 层全收敛，观测 5.0 偏差 0.08）** |
| 年均 AET | 0（无 ET） | **935 mm**（水分闭合：AET+径流+深层+Δ储=降水） |
| 年均径流 | 34% | ~50%（K(θ) 级联 + ET 前置） |
| 末月表层 pH | ~6.9 | 6.94（仍碳酸缓冲主导） |
| L1 θ 月尾 | 0.275（50% 饱和） | 0.41（θ_FC，湿润区月尾回充） |

**科学诚实记录（E2/E3 方向性验收结论）**：
...
## 九、v0.7.x 工单 77 落地执行日志（2026-08-21，REACTION 电荷平衡修复）

### 9.1 科学发现与证伪（详见 `docs/analysis/V0_7_X_REACTION_CHARGE_POSTMORTEM.md`）

实施原工单 77（GAS_PHASE 动态化）前的 PHREEQC 探针（`output/gas_phase_probe.py`）**证伪原假设**：

| 探针 | 结果 | 结论 |
|------|------|------|
| `-fixed_volume`（有限缓冲，原候选） | 产酸时气相 CO₂ 暴涨 7.3e6、基线 pH 偏离 1.5+ | **IPhreeqc 3.8.6 数值不可靠，弃用** |
| `-fixed_pressure` 产酸 686 mol | pH 5.00→5.05 | GAS_PHASE 吞酸幅度极小，**非碱化主因** |
| 裸 `H+ 1372` | pH 不变（5.00） | 裸 H⁺ 注入**不酸化**（硝化产酸从未生效） |
| 裸 `Ca+2 343` | pH **9.28** | **复现 v0.7.0 fertilizer 碱化**——裸阳离子伪碱化是主因 |
| 配对 `H+ + An-` / `Ca+2 + 2An-` | pH 2.99 / 5.00 | **电荷配对修复有效** |

**根因**：PHREEQC 平衡强制电荷中性——裸阳离子注入 → 引擎产生 OH⁻ 伪碱化；
裸 H⁺ 注入 → 氧化还原缓冲吞没不酸化。追溯到 **v0.3.0 硝化产酸裸 H⁺**（spec 05/工单 06，
未实测酸化效果）与 **v0.7.0 NH₄⁺ 置换裸盐基**（工单 72）。

### 9.2 修复（工单 77，charge pairing）

净电荷 REACTION 注入按等当量伴随保守惰性阴离子 `An⁻`（复用 companion inert_anion 机制）：
硝化产酸/置换盐基/companion acid/钾镁肥/预平衡锚定。新配置
`simulation.charge_pairing.{enable/anion}`（默认启用）。

### 9.3 验证

- **单层施肥月 pH 4.87 不碱化**（伪碱化机制修复实证，溶液 Ca 3e-5 不累积）
- **sensitivity fertilizer 3y（无 weathering）**：pairing ON 10.86 vs OFF（v0.7.0 基线）11.20（首年 7.56 vs 9.09）
- **333 测试全绿**（324 + 9 新增 `tests/test_charge_pairing.py`）

### 9.4 科学诚实（遗留）

pairing 修复伪碱化；4 层事件驱动下交换相盐基清空 + 盐基滞留溶液淋失不足仍致碱化
（10+）→ **工单 80（lime/盐基淋失强化）**；配对 An⁻ 离子强度 → 工单 78（KNOBS）。
GAS_PHASE 保持 `-fixed_pressure`。
