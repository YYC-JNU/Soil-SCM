...
> **当前版本**：**v0.7.3**（2026-08-31：工单 86 **L4 收敛分层架构**（探针证伪 A/D）+ 工单 D **深层铝缓冲标定 C2 兜底**（铝/pCO₂ 通道证伪，L4 5.885 已接受偏差）；**380 测试**）+ **v0.7.2**（2026-08-31 更新发布：工单 83~87，含 30y 权威基线 + P0-C + 分层架构）+ 地球化学深化（工单 77/78/80，见 §十一）
...
### v0.7.3（2026-08-31：工单 86 + 工单 D 收口）

> - **工单 86 L4 收敛性能优化（探针证伪 A/D + 分层架构）**：KNOBS 分层迭代
>   （深层 500→1000）实测 **+56.5% 更慢且重试不减少**——PHREEQC 对首次迭代
>   预算非预期敏感（首次 500+重试 1000 收敛 vs 首次 1000 需重试 2000）；
>   分层传参架构已落地（`layer_index` 全链透传，默认全层 500 与 85 逐位一致）；
>   逐位一致：探针 max|ΔpH|=0、0/48 不一致、phreeqc_ok=1；30y <50 min 需
>   条件数改善后重估（详见 `V0_7_x_L4_CONVERGENCE_PERF.md`）
> - **工单 D 深层铝缓冲标定（C2 兜底）**：两候选通道**均证伪**——①铝缓冲通道
>   （解开 MINERAL_SCALE + log_k 微调）机制性证伪：预平衡观测锚定（注入配对
>   阳离子+An⁻ 提供 Al³⁺ 阴离子通道）→ 物理量 Al(OH)₃(a) 初始化耗尽
>   （`AlOH3a=0`，log_k 8.8~10.8 均成立）→ 30y 无缓冲；②pCO₂ 碳酸缓冲通道
>   30y 动态证伪：回升期部分有效（y15=4.578）但后期被深层盐基碱化（L3→6.45）
>   推回 5.91，pCO₂ 0.04/0.05 轨迹几乎逐位相同（护栏 [0.02,0.05] 无可达 [4,5] 值）。
>   **L4 pH 5.885 记为已接受偏差**，权威基线保持 v85 不升级；铝 + pCO₂ 分层
>   机制保留为"机制预留"（默认 = v85 基线）；**natural 先行铁律机器化**
>   （sensitivity `--guard-natural`）；长期 KINETICS 铝缓冲工单登记 backlog
>   （详见 `V0_7_x_DEEP_AL_BUFFER_CALIBRATION.md`）
> - **测试**：**380 passed**
> - **⚠️ 科学诚实**：工单 86 性能目标（<50 min）与工单 D 目标（L4 4~5）均未
>   达成，如实记录证伪链；两机制保留为预留，条件成熟（KINETICS / 真实剖面数据）
>   可重启

### v0.7.2（2026-08-31 更新发布：工单 83 ~ 87 + 工单 86 收口）
...
> - **工单 85 30y 全量重跑 + verify（权威基线）**：**8 情景 30y 全部 `phreeqc_ok=1`**
>   （`sensitivity_pH_30yr_v85.csv`）；verify ①②③⑤ PASS，④ 排序 FAIL 归因 natural
>   中期碱化伪影（N 5.21 > L 5.09，非 lime 缺陷）；排序判据修正 **F<N<L**；
>   权威基线锁定（供工单 86 逐位基准 + D 工单最终复跑对照）（详见
>   `docs/analysis/V0_7_x_30YR_FULL_RERUN_VERIFY.md`）
> - **工单 87 深层极端酸化物理化（P0）**：H0 决定性归因（BS 分母不含 HX → AlX3
>   耗尽后 BS 100% 伪影 → E_base 恒全量 An⁻ 泵）；**P0-C BS 含 HX 物理口径** +
>   **P0-A 盐基保底护栏**（`c_floor_mmol_L`）；**30y 复测 L4 跌穿 1.67→5.885**
>   （护栏复核 [3.5, 5.0]，5.9 为已接受已知偏差，D 工单精修）+ HX 膨胀减缓 62%
>   （详见 `docs/analysis/V0_7_x_DEEP_ACIDIFICATION_PHYSICALIZATION.md`）
> - **工单 86 L4 收敛性能优化（探针证伪 A/D + 分层架构）**：KNOBS 分层迭代
>   （深层 500→1000）实测 **+56.5% 更慢且重试不减少**——PHREEQC 对首次迭代
>   预算非预期敏感（首次 500+重试 1000 收敛 vs 首次 1000 需重试 2000）；
>   分层传参架构已落地（`layer_index` 全链透传，默认全层 500 与 85 逐位一致）；
>   逐位一致：探针 max|ΔpH|=0、0/48 不一致、phreeqc_ok=1；30y <50 min 需 D
>   工单改变 L4 条件数后重估（详见
>   `docs/analysis/V0_7_x_L4_CONVERGENCE_PERF.md` + 探针 `output/probe_86_layer_iters.py`）
> - **测试**：**365 passed**
> - **⚠️ 科学诚实**：工单 84 探针证实工单 83 的"碱化缓解预期"不成立（L1 轨迹与
>   物理化前逐位一致）；30y 中期碱化 ~8 为表层机制伪影，修复候选转工单 79/81 或
>   新工单；工单 86 性能目标未达成（<50 min），如实记录并留 D 工单后重估
