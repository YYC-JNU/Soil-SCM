...
> **当前版本**：**v0.7.5**（400 测试；含**只读交换相质量守恒诊断** — 交换相零化/塌陷可观测化，D4/D5 类静默垃圾解首次可检出；另含迭代预算降本 — KNOBS 首次迭代 500→100、重试下限 500→0，8 情景 30y 逐位一致）
...
> **说明**
> - 化学计算依赖官方 `phreeqc` 包（IPhreeqc 3.8.6，USGS 官方引擎）。
...
│   └── texture_code.json       # 土壤质地编码表（卡钦斯基制）
├── src/                        # 源码
│   ├── __init__.py
...
| `simulation` | `n_layers` | `1` | 分层数：`1`=单层，`4`=多分层 |
| `simulation` | `layer_depths` | `None` | 每层厚度 cm：长度必须 = n_layers，派生每层 `effective_depth` |
| `simulation` | `layer_overrides` | `[]` | 逐层参数覆盖：长度必须 = n_layers，覆盖 ph/有机质/CEC/容重/交换性离子/pCO2/矿物；`n_layers=1` 时忽略 |
| `simulation` | `enable_surface` | `false` | SURFACE 表面络合：`true`=启用 Hfo_s/Hfo_w 铁氧化物表面，P/Zn 吸附生效 |
| `simulation` | `enable_pre_equilibration` | `true` | 初始状态预平衡（建议保持开启） |
| `simulation` | `pre_equilibration_max_steps` | `60` | 预平衡最大迭代步数 |
| `simulation` | `nitrification_k1` | `1.0` | 尿素水解速率（/月；`0~1`，1.0=当月全水解） |
| `simulation` | `nitrification_k2` | `0.4` | 硝化速率（/月；`0~1`，NH₄⁺→NO₃⁻ 比例） |
| `simulation` | `hydrology_seed` | `42` | 随机降雨种子（同 seed 可复现） |
| `simulation` | `layer_overrides` 水文 | — | 逐层水文字段：`clay_pct`/`porosity`/`ksat`/`ksat_surface`/VGM 参数；`n_layers=4` 自动启用内置默认 |
| `simulation` | `sub_time_step_days` | `0` | 子时间步长（天）：`0`=关闭，`1~7`=启用；水文模式不适用 |
...
| `precipitation_chemistry` | `ph` / `ions` | `-1` | 内联降水化学：全部 -1 回退 JSON；全部有效值覆盖；混合报错 |
| `output` | `directory` | `./output` | 输出目录 |
| `output` | `format` | `csv` | 输出格式：`csv` / `netcdf`（未装 netCDF4 时回退 CSV） |
| `output` | `variables` | 见第七节 | 输出变量列表 |
...
时间序列包含时间列（`year`、`month`、`time_decimal`）与以下变量（由 `config.output.variables` 控制）：
...
1. **Al(OH)₄⁻ 两性溶解**：pH 升高后总 Al 浓度反而上升——Al 以铝酸根（Al(OH)₄⁻）形态碱性溶解，Al³⁺ 实际剧降（pH 10 时 ~10⁻²³）。
2. **矿物量折中**：矿物量取物理值 0.001（`mineral_scale`），以避免矿物量大导致的碱性突变，但压缩了矿物缓冲容量。
3. **SURFACE 与雨季交互**：启用 SURFACE（`enable_surface: true`）后，Hfo 表面质子化在雨季强入渗时加速交换 Al 耗尽，pH 上升更快——建议与多分层配合使用，独立启用会加剧。
4. **PHREEQC 无法维持溶液无机氮形态**：`phreeqc.dat` 的 N 氧化还原平衡将注入溶液的无机氮（NH₄⁺/NO₃⁻）热力学平衡为 N₂。模型采用**库存层**方案（氮形态为模型状态，硝化产酸 2H⁺ 注入 REACTION），这是既有局限的显式化。
5. **单层 fertilizer 长期 AlX₃ 耗尽→pH 突升**：弱产酸下 AlX₃ 被盐基置换 + 排水淋失耗尽 → pH 突升 ~10。**建议**：fertilizer 情景使用多层（`n_layers=4`），逐层垂直缓冲可推迟耗尽。
...
| 扩展方向 | 说明 |
|----------|------|
| 根系吸水 / 蒸散发 | Feddes ET + Oudin PET 通道已实现；根系吸水深度分布待细化 |
| 有机质分解 | OM 矿化产 CO₂ 模块已实现；完整碳循环待扩展 |
| 更多肥料类型 | 支持复合肥、缓释肥等 |
...
- van Genuchten M.Th. A closed-form equation for predicting the hydraulic conductivity of unsaturated soils. SSSAJ, 1980, 44(5): 892-898.
- Oudin L. et al. Which potential evapotranspiration input for a lumped rainfall-runoff model? Journal of Hydrology, 2005, 303(1-4): 290-306.
