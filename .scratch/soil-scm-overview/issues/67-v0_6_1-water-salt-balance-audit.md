...
- [ ] `tools/water_salt_balance.py`：读取主 CSV + event 明细，计算逐月/全年水量闭合（P − Runoff − AET − Qlat − Qbase − ΔS，|<1%|）与盐分进出口对账（输入降水盐分 vs 各出口盐分 + 存储变化，|<5%|）
- [ ] 输出闭合报表（CSV/控制台）：每月水量项、盐分项、闭合残差、是否超阈值告警
- [ ] 30 年 8 情景闭合验证跑通（后台进程，避免工具 30s 超时）
- [ ] 测试（S7）：构造已知收支样例验证闭合公式、阈值告警逻辑
