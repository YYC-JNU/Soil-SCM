...
#   - log_k=3.0: v0.6.1 扫描标定 (2026-08-20 实测): log_k=1.0 → 平衡 pH 3.74 (酸解离过强,
#     预平衡无法锚定观测 5.0); log_k=3.0 → 平衡 pH 4.99 (自然收敛观测, HX 保留为
#     真实酸库); log_k≥5.0 → pH 过高 (≥6.9) 且 HX 膨胀超 CEC。见 spec 62 风险注记。
#   - v0.7.0 工单76 调优 B (2026-08-21): log_k 3.0→2.8 — E2 判别与 natural 短程实测
#     暴露 log_k=3.0 锁酸过强 (交换相盐基首月清空 + natural 碱化加速 + PET 敏感性
#     传导减弱), 科学解读 §一 机制 C 建议区间 2.5~2.8, 最终取 2.8 (区间内偏保守;
#     验收实测预平衡收敛 4.86, 见 v0.7.x 工单 81)。
#   - GAP_H_FRACTION: CEC 缺口三通道重分配 (HX/AlX3/NaX), 与 GAP_AL_FRACTION 并列
HX_LOGK = 2.8               # HX 交换物种 log_k (工单76 调优 B: 3.0→2.8, 科学解读区间 2.5~2.8)
...
# L3+L4 (L4 单层 71%), 重试步占 84% 模拟时间; 现状难步 = "首次 500 超限 → 重试
# 1000" 双跑 (1500 迭代 + 2 次 RunString)。
# ⚠️ 探针证伪 (2026-08-31, probe_86_layer_iters.py natural 1y): 深层基础迭代
# 500→1000 **实测更慢 (+56.5%) 且重试次数不变 (99)** — PHREEQC 对首次迭代预算
# 有非预期敏感性: 同一批 L4 难步, 首次 500+重试 1000 收敛, 但首次 1000 仍超限需
# 重试 2000 (迭代总成本 500+1000=1500 → 1000+2000=3000)。"首次即高迭代消除
